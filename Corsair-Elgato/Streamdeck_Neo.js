export function Name() { return "Elgato Stream Deck Neo"; }
export function VendorId() { return 0x0fd9; }
export function ProductId() { return 0x009A; }
...
export function ControllableParameters() {
    return [
        ...
        { "property": "hwbrightness", "group": "", "label": "Hardware Brightness", "step": "1", "type": "number", "min": "1", "max": "100", "default": "100" },
...
const vLedNames = ["LED 1", "LED 2", "LED 3", "LED 4", "LED 5", "LED 6", "LED 7", "LED 8", "Info Bar"];
const vLedPositions =
    [
        [0, 0], [1, 0], [2, 0], [3, 0],
        [0, 1], [1, 1], [2, 1], [3, 1],
...
function colorgrabberSingle(shutdown = false) {
    for (let iIdx = 0; iIdx < vLedNames.length - 1; iIdx++) {
        ...
        if (shutdown) {
            ...
            const friendlyX = iIdx % 4;
            const friendlyY = Math.floor(iIdx / 4);
            color = device.color(friendlyX, friendlyY);
        }
...
function bigScreenGrabberSingle(shutdown = false) {
    ...
    if (lastButtonRGB[8][0] !== color[0] || lastButtonRGB[8][1] !== color[1] || lastButtonRGB[8][2] !== color[2]) {
        ...
        // Adjusted for Stream Deck Neo info bar resolution
        RGBData = device.ConvertColorToImageBuffer(buttoncolor, 248, 58, "JPEG");
...
function bigScreenGrabber() {
    ...
    // Adjusted for Stream Deck Neo info bar resolution
    RGBData = device.getImageBuffer(0, stripY, safeW, safeH, { flipH: false, outputWidth: 248, outputHeight: 58, format: "JPEG" });
...
export function ImageUrl() {
    return "https://raw.githubusercontent.com/SRGBmods/public/unsupported-experiments/_images/StreamdeckNeo.png";
}