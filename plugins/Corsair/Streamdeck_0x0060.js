...
function grabColors(shutdown)
{
	for(let iIdx = 0; iIdx < vLedNames.length; iIdx++)
	{
		...
		if(lastButtonRGB[iIdx][0] !== color[0] || lastButtonRGB[iIdx][1] !== color[1] || lastButtonRGB[iIdx][2] !== color[2])
		{
			...
			rgbdata = device.ConvertColorToImageBuffer(buttoncolor, 72, 72, "BMP");
...
{
	for(let packetCount = 1; packetCount <= 2; packetCount++)
	{
		let packet =
		[
			0x02, 0x01, packetCount, 0x00, packetCount-1, iIdx+1, 0x00, 0x00,
